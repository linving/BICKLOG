                ...
                case KEY_ENTER:
                    if(selected > -1) {
                        ...
                        if(handleSelection(value)) {
                          var old = obj.value;
		                  //delete notcompleted text after comma
		                  var lastIndexOfComma = old.lastIndexOf(',');
		                  old = old.substring(0, lastIndexOfComma+1);
		                  obj.value = old+value+',';
        ...
        if(element.firstChild && element.firstChild.childNodes) {
            ...
            var clickFunc = function(event) {
                ...
                if(value = handleSelection(value)) {
                  //----------- TANERDILER CUSTOMIZATION -----------
                  var old = wicketGet(elementId).value;
                  //delete notcompleted text after comma
                  var lastIndexOfComma = old.lastIndexOf(',');
                  old = old.substring(0, lastIndexOfComma+1);
                  wicketGet(elementId).value = old+value+',';
